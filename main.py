...
bold_font = pg.font.SysFont("Arial", 22, bold=True)

pass_rect = pg.Rect(BOARD_VIEW_SIZE + 20, 150, 140, 45)
resign_rect = pg.Rect(BOARD_VIEW_SIZE + 20, 220, 140, 45)
quit_dashboard_rect = pg.Rect(BOARD_VIEW_SIZE + 15, 280, 150, 50)
...
def draw_board():
    """ Draws Game Grid, Menu, and Stones. 
    ...
    if not game_over:
        ...
        txt_status = bold_font.render(status_msg, True, RED if game_winner == 'Bot' else GREEN)
        screen.blit(txt_status, (BOARD_VIEW_SIZE + 15, 160))
        ...
        if event.type == pg.QUIT:
            ...
            if not game_over:
                ...
                if 0 <= row < BOARD_SIZE and 0 <= col < BOARD_SIZE:
                    ...
                    status_msg = "You resigned"
                    game_over = True
                    game_winner = 'Bot'
            else:
                if quit_dashboard_rect.collidepoint((mouse_x, mouse_y)):
                    running = False

    # backend data pipeline
    if game_over and not already_logged and not force_quit_no_log:
        draw_board()
        pg.display.flip()
        ...
        # Take clean subsurface crop while window frame pixels are 100% active
        board_rect = pg.Rect(0, 0, BOARD_VIEW_SIZE, WINDOW_HEIGHT)
        board_surface = screen.subsurface(board_rect)
        
        img_buffer = io.BytesIO()
        pg.image.save(board_surface, img_buffer, "png")
        raw_png_bytes = img_buffer.getvalue()
        img_buffer.close()
        
        # Log metadata row into our database warehouse files
        log_game(
